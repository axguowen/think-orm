...
trait ModelRelationQuery
{
     ...
     * @return $this
     */
    public function withSearch($fields, $data = [], $prefix = '')
    {
        if (is_string($fields)) {
     ...
     * @return $this
     */
    public function has($relation, $operator = '>=', $count = 1, $id = '*', $joinType = '')
    {
     ...
     * @return $this
     */
    public function hasWhere($relation, $where = [], $fields = '*', $joinType = '')
    {
        ...
        foreach ($this->options['json'] as $name) {
            ...
            if (isset($withAttr[$name])) {
                foreach ($withAttr[$name] as $key => $closure) {
                    $jsonData[$key] = $closure(isset($jsonData[$key]) ? $jsonData[$key] : null, $jsonData);
                }
            }
        ...
        foreach (['with', 'with_join'] as $with) {
            ...
            if (!empty($this->options[$with])) {
                $result->eagerlyResultSet(
                    ...
                    isset($this->options['with_cache']) ? $this->options['with_cache'] : false
                );
            }
        }
