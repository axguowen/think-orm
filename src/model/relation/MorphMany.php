...
class MorphMany extends Relation
{
     ...
     * @return Query
     */
    public function has($operator = '>=', $count = 1, $id = '*', $joinType = '', Query $query = null)
    {
     ...
     * @return array
     */
    protected function eagerlyMorphToMany(array $where, array $subRelation = [], Closure $closure = null, array $cache = [])
    {
        // 预载入关联查询 支持嵌套预载入
        ...
        $list = $this->query
            ->where($where)
            ->with($subRelation)
            ->cache(isset($cache[0]) ? $cache[0] : false, isset($cache[1]) ? $cache[1] : null, isset($cache[2]) ? $cache[2] : null)
            ->select();
        $morphKey = $this->morphKey;
